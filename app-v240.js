...
async function registerServiceWorker() {
  ...
  try {
    ...
    const registration = await navigator.serviceWorker.register("./sw.js?v=258");
...
async function createProfessionalScannedCanvas(sourceCanvas, options = {}) {
  ...
  try {
    const region = await detectDocumentRegion(sourceCanvas, options);
    if (region?.points?.length === 4) {
      const rectified = await rectifyDocumentPerspective(sourceCanvas, region, options);
      if (rectified?.canvas && isUsableScannedCanvas(rectified.canvas, sourceCanvas)) {
        result = {
  ...
  try {
    const box = await detectPrimaryPageContentBox(result.canvas, { conservative: false });
    const cropped = await cropToPrimaryPage(result.canvas, box, options);
...
function getPendingDeskewLabel(page) {
  ...
  if (normalizeScanMode(page.scanMode) !== SCAN_MODE_HD) {
    return "";
  }
  if (meta.lineStraightenApplied && Number(meta.lineStraightenCorrectedRows) > 0) {
    return ` · 已扶正 ${Number(meta.lineStraightenCorrectedRows)} 行`;
  }
  return "";
}
